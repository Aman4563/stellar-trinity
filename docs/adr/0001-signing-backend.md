...
The constraint that shapes the choice is Trinity's zero-install property. README.md:17 states there is nothing to install and no library to import, because a contract is pasted into a coding agent and the agent is the runtime. pyproject.toml:8 pins `dependencies = []`. The `Prove the harness has no runtime dependencies` step at `.github/workflows/ci.yaml`:44 turns that one line into a proof: it loads `pyproject.toml` with `tomllib` and exits non-zero unless the declared dependency tables are empty. A signing backend that imports a third-party package fails that step and falsifies the README in the same commit.
...
Extend the CI dependency-freedom assertion so it covers `[project.optional-dependencies]` as well as `[project].dependencies`. The proof invoked from `.github/workflows/ci.yaml`:44 must read both tables and fail when either carries a package.

## Options considered

| Option | Verdict | Reasoning |
|---|---|---|
| (a) `cryptography` or `pynacl` as a real Python dependency | Rejected day-one | Breaks the stated zero-install property. README.md:17 says you adopt Trinity by pasting and there is nothing to install. Adopting it forces an edit to the `Prove the harness has no runtime dependencies` step at `.github/workflows/ci.yaml`:44, so the change would have to weaken the proof that guards the property it violates. |
