...
```json
{
  ...
  "roles": [
    ...
    {"name": "sentinel_clearer", "threshold": 1}
  ],
  "principals": [
    {
    ...
    },
    {
      ...
      "roles": ["execution_operator", "gate_approver", "sentinel_clearer"]
    },
    {
      "name": "retired-operator@example.test",
      "roles": ["execution_operator"]
    }
  ],
  "revocations": [
...
`principals` binds each unique principal name to one or more defined roles. An optional `expires` value limits that principal independently of the root. The parser rejects duplicate principal names, repeated role bindings, empty role lists, and bindings to undefined roles. Public keys or backend trust entries live in the signature backend policy, such as an SSH allowed signers file; the root binds the identity returned by that backend rather than embedding key bytes. Group-authorized v3 evidence requires exactly one explicit principal per software key in `allowed_signers`, rejects wildcard or comma-separated principal forms and duplicate public key material across identities, and does not support certificate or cert-authority entries.

`sentinel_clearer` at threshold 1 authorizes a sentinel clearance, the DSSE envelope `tools/sentinel.py` requires before an uncleared `SAB_REPEATED_ATTEMPT` alert stops capping the disposition at BLOCK. The alerted actor's own verified principal is supplied as `excluded_principals`, so a clearance signed by the key the alert names is refused as `self_approval` before a byte is written and again when a planted one is read, which makes self-clearance a cryptographic refusal instead of a naming convention. A principal bound to `sentinel_clearer` alone gains no disposition or release authority, because that authority is bound to `gate_producer` and `gate_approver` under the separately governed release policy.
...
A principal who signs Trinity attestations and also signs git commits must be enrolled in `allowed_signers` as `namespaces="trinity.attestation.v1,git"`, because the sentinel verifies the signature on HEAD under the `git` namespace to attribute an attempt to a verified principal.

A principal scoped to the attestation namespace alone signs commits this repository cannot attribute, so their attempts land on the claimed identity `unverified:<author>`, and a claimed identity names no verified principal to exclude from a clearance.
