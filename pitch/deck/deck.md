...
A curated reading list of 21 anchor papers motivates the principles, reported as counts and labeled a selection, not a survey.

*Trace: `total_anchors = 21` in `paper/data/corpus.json`, verified by the `paper/data/corpus.py` self-check.*

---

## Slide 8: Where the anchors cluster

The 21 anchors span 7 themes, and the largest is benchmark construction and hardening at 6 of 21.

![Theme distribution](../figures/theme_distribution.png)

*Trace: `total_anchors = 21`, `theme_count = 7`, and `benchmark_construction_and_hardening = 6/21` in `paper/data/corpus.json`; figure reused from `paper/figures/theme_distribution.png`.*

---

## Slide 9: The pressure is current

Of the 21 anchors, 18 were published in 2026, 2 in 2025, and 1 in 2024. The evidence motivating the standard comes from the live frontier, not a settled literature.

*Trace: year counts `2026 = 18/21`, `2025 = 2/21`, `2024 = 1/21` in `paper/data/corpus.json`.*
...
Every FORGE invocation opens one batch of thirty ordinary task slots plus at most one anchor, worked as parallel lanes, with Frontier-defeat as the default design target unless a human grant exempts a slot. The batch spreads coverage across eligible archetypes AR1 to AR10, no archetype claims more than one fifth of the slots, and an underfilled or overanchored batch is a named gap, never a quiet shortfall.

*Trace: `FORGE.md` non-negotiable rules 10 and 11 and Phase 0 step 7 bind thirty ordinary slots plus at most one anchor, parallel lanes, the Frontier-defeat mandate and grant exception, and the archetype spread.*
...
*Trace: `ENGRAM.md` Phase 1 steps 8b, 8g, and 8h and `FORGE.md` Phase 0 steps 9h through 9j pin the four screening roots, the named baseline rows, the union authority mode, and the canary normalization domain.*
...
## Slide 19: Target and composition headroom are distinct

Hardness targets require at least sixty percent headroom: every target sits at or above 1.6 times the strongest anchored frontier result it cites. Author-side composition separately requires at least fifty percent headroom, or 1.5 times every governing floor, rounding up: twelve instrument-bound levers across nine categories compose to at least eighteen levers across fourteen. Both are design obligations, never evidence of difficulty.

*Trace: `ENGRAM.md` Phase H step 6 binds the 1.6 target multiple; `FORGE.md` Hardness rules 2 and 2e bind the 1.5 composition multiple and rounded floors; `paper/main.md`, section 4, Pattern 1 restates both.*
...
## Slide 24: Erratum: the superseded corpus round

Erratum. Reconciliation against the checked-in reading list found an omitted paper, OpenAnt (Korda and Evron, 2026). Superseded round: anchors = 20; papers in 2026 = 17; agentic case studies = 1. Current round: anchors = 21; papers in 2026 = 18; agentic case studies = 2. The 7 themes and largest theme count of 6 are unchanged. These superseded counts are history, not current headlines.

*Trace: `paper/main.md`, section 5.3, Erratum records these prior and current counts; `paper/data/corpus.json` supplies the current headlines; `ENGRAM.md` Phase S step 17 requires this history on the pitch surface.*
