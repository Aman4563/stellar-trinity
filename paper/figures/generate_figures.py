...
def save_both(fig: plt.Figure, stem: str) -> None:
    fig.savefig(FIGURES_DIR / f"{stem}.pdf", dpi=300, metadata={"CreationDate": None})
    fig.savefig(FIGURES_DIR / f"{stem}.png", dpi=300, metadata={"Software": None})
    plt.close(fig)
...
def figure_theme_distribution(aggregates: dict) -> None:
    ...
    fig.tight_layout()
    save_both(fig, "theme_distribution")
...
def figure_evidence_lifecycle() -> None:
    ...
    fig.tight_layout()
    save_both(fig, "evidence_lifecycle")
