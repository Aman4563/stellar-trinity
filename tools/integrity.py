...
if TYPE_CHECKING:
    from tools import harness_config, layout, pilot
    ...
    from tools.epochs import check_epochs
    from tools.gate import CHECKS as GATE_CHECKS
    from tools.harbor import CHECKS as HARBOR_CHECKS
    from tools.judge_reliability import check_judge_reliability
    from tools.pilot import check_pilot_attempt_registry
    from tools.pipeline import CHECKS as PIPELINE_CHECKS
    ...
    from tools.release import CHECKS as RELEASE_CHECKS
    from tools.sabotage import CHECKS as SABOTAGE_CHECKS
    from tools.safe_git import SafeGitError, SafeGitRepository
    from tools.sentinel import CHECKS as SENTINEL_CHECKS
else:
    ...
    try:
        ...
        from epochs import check_epochs
        from gate import CHECKS as GATE_CHECKS
        from harbor import CHECKS as HARBOR_CHECKS
        from judge_reliability import check_judge_reliability
        from pilot import check_pilot_attempt_registry
        from pipeline import CHECKS as PIPELINE_CHECKS
        ...
        from release import CHECKS as RELEASE_CHECKS
        from sabotage import CHECKS as SABOTAGE_CHECKS
        from safe_git import SafeGitError, SafeGitRepository
        from sentinel import CHECKS as SENTINEL_CHECKS
...
# regenerated from .memory/hardness.yaml, and TRACKING.md, the GENERATED
# sectioned executive tracker each of the three instruments updates at its own
# section on every invocation while ENGRAM scaffolds it and owns its shape, so
# the "no split brain" rule stays satisfiable without widening REPORTS into a
# one-to-many map.
EXTRA_REPORTS: dict[str, list[str]] = {"ENGRAM.md": ["HARDNESS.md", "TRACKING.md"]}
...
BUNDLE_ROOTS: tuple[str, ...] = (STAGING_ROOT, *LANE_ROOTS)
...
# Every door is written from its template by ``gate.py install`` at the preflight and
# is never hand-authored; a door whose bytes differ from its template is drift.
DOOR_TEMPLATES_DIR: Path = Path(__file__).resolve().parent.parent / "templates" / "doors"
FRONT_DOOR_CONTRACTS: dict[str, str] = {
    **{f".opencode/commands/{name}": "ENGRAM.md" for name in PARENT_COMMAND_DOORS[:-3]},
    ".opencode/commands/maestro.md": "MAESTRO.md",
    ".opencode/commands/forge.md": "FORGE.md",
    ".opencode/commands/crucible.md": "CRUCIBLE.md",
    ".opencode/commands/harness.md": "FORGE.md",
    ".agents/skills/seal/SKILL.md": "ENGRAM.md",
    ".agents/skills/temper/SKILL.md": "FORGE.md",
    ".agents/skills/assay/SKILL.md": "CRUCIBLE.md",
    ".agents/skills/rig/SKILL.md": "FORGE.md",
}


def door_template_path(relative: str) -> Path:
    """The template under ``templates/doors/`` that ``gate.py install`` writes at ``relative``."""

    if relative.startswith(".opencode/commands/"):
        return DOOR_TEMPLATES_DIR / "commands" / relative.rsplit("/", 1)[1]
    return DOOR_TEMPLATES_DIR / "skills" / Path(relative).parts[2] / "SKILL.md"
...
MIGRATION_METADATA_PREFIXES: tuple[str, ...] = (
    ".trinity/migrations",
    ".trinity/project.json",
)

# The receipts ``trinity/tools/gate.py install`` writes at the parent root: one branch
# protection read-back per gated repository. Machine-written, never a harness root.
INSTALL_ROOT: str = ".trinity-install"
...
# ENGRAM Phase H step 2: the eight source-class tiers of the trust ladder. A
# tier is a default prior on where evidence came from, never a row state, never
# a lever state, and never a disposition; never merge these sets.
SOURCE_TIERS: set[str] = {"T0", "T1", "T2", "T3", "T4", "T5", "T6", "T7"}
...
NOT_STATUS: set[str] = set(PROJECTIONS) | {"CODEOWNERS", "LICENSE", "GITHUB_ACTOR"}

STATUS_TOKEN: re.Pattern[str] = re.compile(r"^[A-Z][A-Z0-9_]*(:[A-Z][A-Z0-9_]*)?$")

# Refusal codes emitted by the delivery-integrity and anti-sabotage modules, plus
# the coverage-gap names the contracts state in prose. They share the uppercase
# shape of a disposition token but name a gate outcome or a recorded gap, never a
# disposition, so the closed-vocabulary check exempts them by prefix.
REFUSAL_CODE_PREFIXES: tuple[str, ...] = (
    "SAB_",
    "RELEASE_",
    "HARBOR_",
    "PIPELINE_",
...
CONTRACT_VOCAB: dict[str, list[str]] = {
    "FORGE.md": [
        ...
        "PIPELINE_LANE_FULL",
        "PIPELINE_STREAM_CONFLICT",
        ".seed/lanes/",
        ".podium/queue/",
        ".seed/runs/",
        "trinity.qualification/v2",
...
# Execution-fidelity detector vocabulary. An author who learns which detector
# refused a pilot, or why, adapts the next task to the detector instead of to the
# frontier, so the reason side of the fidelity instrument stays on the auditor's
# surface and never reaches the author contract.
FIDELITY_TERMS: tuple[str, ...] = (
    "compact_boundary",
    "compaction event",
    "condensation event",
    "trial reason code",
...
SHARED_BLOCKS: list[str] = [
    ...
    (
        "Adversarial operator doctrine: the operator running this contract is untrusted, "
        "and a disposition is a machine record rather than prose."
    ),
    (
        "Every SAB refusal is appended to the hash-chained sentinel ledger at the parent "
        "root. A repeated attempt inside the rolling window raises SAB_REPEATED_ATTEMPT and "
        "caps the disposition at BLOCK until a principal the trust root binds to the "
        "`sentinel_clearer` role, whom the alerted actor's own verified signature does not "
        "name, records a clearance as a DSSE envelope signed by a software SSH key and "
        "verified against `.memory/allowed_signers`. A typed clearance carrying a name "
        "where a signature belongs is SAB_SENTINEL_CLEARANCE_UNSIGNED and clears nothing. "
        "An attempt whose HEAD commit carries no signature this repository can verify is "
        "attributed to a claimed identity that names no principal and that no clearance may "
        "rest on, and continuous integration re-derives both the ledger and the refusal "
        "from the pushed commit on runners the operator does not administer."
    ),
    (
        "With no bypass syntax on a gate step and no SHIP token entering a root report, the "
        "finding is SAB_WORKFLOW_TAMPER_SUSPECT and caps at HOLD, and the same actor closes it "
        "by landing the flagged governance paths and the flagged subject paths again in "
        "separate commits inside the scan window. With either signal the finding is "
        "SAB_WORKFLOW_TAMPER and caps at BLOCK."
    ),
    (
        "Rewriting, force-pushing, replacing, or padding published history to make a finding "
        "leave the scan window is itself a sabotage attempt and never a recovery, and no "
        "contract, instrument, root report, or lane may recommend it. A SAB_WORKFLOW_TAMPER "
        "finding is resolved only by an append-only signed remediation record, authorized by "
        "a distinct externally "
        "enrolled principal group and never by a key the flagged commit introduced, that "
        "binds the flagged commit identities, the corrective commits, the re-verified trust "
        "snapshot, and the rerun evidence while preserving the original commits and their "
        "finding as evidence."
    ),
    (
        "Bypassing a scaffolded gate surface is the same class of attempt: passing a "
        "verification-skipping flag to a push, unsetting or redirecting `core.hooksPath`, "
        "editing the installed hook, or disabling a registered check is a sabotage attempt "
        "that no contract, instrument, root report, or lane may recommend. The only "
        "lawful path past a refusing hook is to fix the finding it names, or to rerun the "
        "preflight so `trinity/tools/gate.py install` refreshes a hook that drifted from "
        "its template."
    ),
    (
        "The preflight installs the parent's workflows, pre-push hook, CODEOWNERS lines, and "
        "branch protection through `trinity/tools/gate.py install` and records a receipt per "
        "repository; a protection it cannot apply is a named coverage gap capping at HOLD, "
        "never a step the operator performs by hand."
    ),
    (
        "Gate preflight and report binding: after the Trinity freshness gate and before any "
        "phase work, run the parent gate at moment preflight through `trinity/tools/gate.py`, "
        "and run it again at moment report before writing the root report."
    ),
    (
        "a report that states a higher token than that record is GATE_DISPOSITION_MISMATCH, "
        "and a run that cannot execute the gate stops before phase work with the failure "
        "recorded as a named blocking coverage gap."
    ),
    (
        "After the Trinity freshness gate and before the resume preflight or any phase "
        "work, every authoring preflight runs `trinity/tools/migrate.py` through the parent "
        "gate."
    ),
    (
        "Collaborative signing doctrine: existing valid individual "
        "`trinity.release-disposition/v2` payloads and envelopes remain accepted as their "
        "exact bytes and are never reserialized into a newer schema."
    ),
    (
        "Every deterministic step this preflight names is implemented once under "
        "`trinity/tools/`; run it there, never re-implement it, never hand-write a file it "
        "emits, and record a hand-written substitute as the named coverage gap "
        "`TOOL_REIMPLEMENTED`."
    ),
]
...
def first_segment(token: str) -> str | None:
    ...
    explicit_project_path = token.startswith("./")
    if explicit_project_path:
        token = token[2:]
    if not token:
        return "." if explicit_project_path else None
    if "/" not in token:
        return token if explicit_project_path else None
...
def check_unknown_paths(path: str, text: str) -> list[Finding]:
    out: list[Finding] = []
    known = set(SHARED_DIRS) | set(BUNDLE_DIRS) | {INSTALL_ROOT}
    ...
    for line_no, token in tokens:
        ...
        if seg == LEGACY_MACHINE_DIR:
            if token.startswith(MIGRATION_METADATA_PREFIXES):
                continue
    ...
    for line_no, line in enumerate(text.split("\n")):
        for span in BACKTICK.findall(line):
            ...
            if not STATUS_TOKEN.match(token) or token in NOT_STATUS:
                continue
            if token.startswith(REFUSAL_CODE_PREFIXES):
                continue
...
def check_fidelity_closure(path: str, text: str) -> list[Finding]:
    """Execution-fidelity detection is auditor-only and never reaches the author contract."""
    if path.rsplit("/", 1)[-1] != "FORGE.md":
        return []
    out: list[Finding] = []
    scoped = list(sentences(text))
    for index, (line_no, sentence) in enumerate(scoped):
        ...
        for term in FIDELITY_TERMS:
            if term in sentence:
                message = (
                    f'author contract names fidelity-detector term "{term}" outside a prohibition'
                )
                out.append(finding(path, line_no, message))
    return out
...
def check_front_doors(root: str, _evaluation_time: datetime | None = None) -> list[Finding]:
    ...
    for relative, contract in FRONT_DOOR_CONTRACTS.items():
        ...
        try:
            expected = door_template_path(relative).read_text(encoding="utf-8")
        ...
        if expected is not None and text != expected:
            out.append(
                finding(
                    door,
                    0,
                    f'front door "{relative}" differs from trinity/templates/doors;'
...
def check_parent_layout(root: str, _evaluation_time: datetime | None = None) -> list[Finding]:
    """Enforce registered parent roots and their repository information boundaries."""
    parent = Path(root)
    metadata = parent / ".git"
    ...
    try:
        sections, malformed = layout.parse_gitmodules(read_text(gitmodules))
    except (OSError, UnicodeDecodeError) as exc:
        sections, malformed = (), (f"unreadable: {exc}",)
    declared: set[str] = set()
    for section in sections:
        path = section.path.strip()
        parts = path.split("/")
        if (
            Path(path).is_absolute()
            or ".." in parts
            or "" in parts
            or (parts[0] in layout.KNOWN_ROOTS and path not in layout.KNOWN_ROOTS)
        ):
            malformed += (f"non-canonical registration path {path!r}",)
        elif path in layout.KNOWN_ROOTS:
            if path in declared:
                malformed += (f"duplicate roster registration {path!r}",)
            declared.add(path)
    if malformed:
        out.append(
            finding(
                gitmodules,
                0,
                f"invalid .gitmodules: {'; '.join(malformed)}; "
                "register each submodule with path and url",
                layout.PARENT_LAYOUT_GITMODULES_INVALID,
            )
        )
    for path in layout.REQUIRED_ROOTS:
        if path not in declared:
            out.append(
                finding(
                    parent / path,
                    0,
                    f"register the required {path} submodule in .gitmodules",
    ...
    for section in sections:
        path = section.path.strip("/").split("/", 1)[0]
        if path not in layout.KNOWN_ROOTS:
            continue
        if section.branch != "main" or section.update != "merge":
            out.append(
                finding(
                    gitmodules,
                    0,
                    f"set {path} branch = main and update = merge in .gitmodules",
                    layout.PARENT_LAYOUT_BRANCH_INVALID,
                )
            )
        if section.url:
            remote = section.url.strip().removeprefix("file://")
            if "://" not in remote and not re.match(r"^[^/:]+:", remote):
                remote = os.path.normpath(remote)
            else:
                remote = remote.lower()
                remote = re.sub(r"^[a-z][a-z0-9+.-]*://", "", remote)
                remote = re.sub(r"^[^@/:]+@", "", remote)
                if "://" not in section.url and re.match(r"^[^/]+:[^/]", remote):
                    remote = remote.replace(":", "/", 1)
            remote = re.sub(r"\.git$", "", remote.rstrip("/")).rstrip("/")
            owners = remotes.setdefault(remote, set())
            for owner in sorted(owners - {path}):
                pair = (min(owner, path), max(owner, path))
                if pair not in pairs:
                    ...
                    out.append(
                        finding(
                            gitmodules,
                            0,
                            f"{owner} and {path} share a remote; "
                            "register distinct submodule remotes",
                            layout.PARENT_LAYOUT_SHARED_REMOTE,
                        )
                    )
            owners.add(path)
    entries: tuple[tuple[str, str, str], ...] | None = None
    try:
        with SafeGitRepository(parent) as repository:
            entries = repository.ls_files_stage()
    except SafeGitError as exc:
        out.append(
            finding(
                metadata,
                0,
                f"index unreadable; layout index coverage is incomplete: {exc}; repair the index",
    ...
    if entries is not None:
        for mode, _oid, path in entries:
            ...
            if mode == "160000":
                gitlinks.add(path)
                if first not in layout.KNOWN_ROOTS and path not in layout.RETIRED_ROOTS:
                    out.append(
                        finding(
                            parent / path,
                            0,
                            f"remove unknown submodule {path} from the parent index",
                            layout.PARENT_LAYOUT_UNKNOWN_SUBMODULE,
                        )
                    )
            elif first in {"samples", "delivery"}:
                tracked.setdefault(first, []).append(path)
            if first in {".secrets", ".trinity-runtime"}:
                out.append(
                    finding(
                        parent / path,
                        0,
                        f"remove leaked path {path} from the parent index",
                        layout.PARENT_LAYOUT_BOUNDARY_LEAK,
                    )
                )
        for path in sorted(declared & layout.KNOWN_ROOTS - gitlinks):
            out.append(
                finding(
                    parent / path,
                    0,
                    f"register {path} as a submodule gitlink, not a plain directory",
                    layout.PARENT_LAYOUT_PLAIN_DIRECTORY,
                )
            )
        for path, paths in sorted(tracked.items()):
            out.append(
                finding(
                    parent / path,
                    0,
                    f"parent tracks {', '.join(paths[:3])}; move bundle bytes into the submodule "
                    "checkout and remove them from the parent index",
                    layout.PARENT_LAYOUT_PARENT_TRACKS_BUNDLE,
                )
            )
    for path in layout.RETIRED_ROOTS:
        retired = parent / path
        try:
            occupied = not retired.is_symlink() and retired.is_dir() and any(retired.iterdir())
        except OSError as exc:
            out.append(
                finding(
                    retired,
                    0,
                    f"retired root unreadable: {exc}; restore access and remove it",
                    layout.PARENT_LAYOUT_SCAN_TRUNCATED,
                )
            )
            occupied = False
        if path in gitlinks or occupied:
            out.append(
                finding(
                    retired,
                    0,
                    f"remove retired {path}; move bundle bytes into the submodule checkout",
                    layout.PARENT_LAYOUT_RETIRED_ROOT,
                )
            )
    for entry in layout.LAYOUT_ROSTER:
        checkout = parent / entry.path
        if checkout.is_symlink():
            out.append(
                finding(
                    checkout,
                    0,
                    "remove the symlink and register a real submodule checkout",
                    layout.PARENT_LAYOUT_SYMLINK,
                )
            )
            continue
        if not checkout.is_dir():
            continue
        # A runner root that is itself a trinity project is another project's root: its
        # own CRUCIBLE writes its .audit/ and its own gate owns its layout, so the parent
        # scan stops at that boundary. Only the runner root earns this; a bundle or
        # memory root vendoring trinity/ is still the leak the roster names.
        if entry.kind is layout.RootKind.RUNNER and layout.is_trinity_project_root(checkout):
            continue
        scan = layout.scan_forbidden(checkout, entry.forbidden)
        if scan.leaks:
            out.append(
                finding(
                    checkout,
                    0,
                    f"remove leaked paths from {entry.path}: {', '.join(scan.leaks[:3])}",
                    layout.PARENT_LAYOUT_BOUNDARY_LEAK,
                )
            )
        for path in scan.symlinks:
            out.append(
                finding(
                    checkout / path,
                    0,
                    f"remove the symlink {path}; checkout boundaries cannot follow it",
                    layout.PARENT_LAYOUT_SYMLINK,
                )
            )
        if scan.truncated:
            out.append(
                finding(
                    ...
                    0,
                    (
                        f"boundary scan unreadable: {', '.join(scan.unreadable)}; "
                        "restore access and rerun the scan"
                        if scan.unreadable
                        else "boundary scan limit reached; reduce entries and rerun the scan"
                    ),
                    layout.PARENT_LAYOUT_SCAN_TRUNCATED,
                )
            )
    ...
    try:
        ignored = {line.strip() for line in read_text(gitignore).splitlines()}
    except (OSError, UnicodeDecodeError):
        ignored = set()
    missing = {".secrets/", ".trinity-runtime/"} - ignored
    if missing:
        out.append(
            finding(
                gitignore,
                0,
                f"add ignored runtime directory lines: {', '.join(sorted(missing))}",
                layout.PARENT_LAYOUT_RUNTIME_UNIGNORED,
            )
        )
    return out
...
def submodule_paths(root: str) -> set[str]:
    ...
    try:
        ...
        return set()
    return set(layout.declared_paths(layout.parse_gitmodules(text)[0]))
...
def approval_gate_policy_path(root_path: Path, *, authority_root: Path | None = None) -> Path:
    authority = authority_root or invocation_root()
    audience = display_project_path(root_path.resolve(), project_root=authority.resolve())
    audience_key = hashlib.sha256(audience.encode()).hexdigest()
    return authority / ".trinity" / "parents" / f"{audience_key}.json"


def _approval_gate_policy(
    root_path: Path,
) -> tuple[attest_replay.GatePolicy | None, Finding | None]:
    authority_root = invocation_root()
    policy_path = approval_gate_policy_path(root_path, authority_root=authority_root)
    try:
        policy = attest_replay.load_gate_policy(policy_path, authority_root=authority_root)
    ...
    expected_audience = display_project_path(expected_root, project_root=authority_root.resolve())
    if policy.expected_audience != expected_audience:
        return None, finding(
            policy_path,
            0,
            "gate-owned approval policy audience does not equal the audited project root; "
            "set expectedAudience to the canonical `./` path of the audited project root "
            "and re-sign attestations over it",
            "APPROVAL_TRUST_ROOT_MALFORMED",
        )
    return policy, None
...
def _execution_statement(
    ...
    version_failure = attest_execution.release_version_failure(payload)
    if version_failure is not None:
        message = (
            f"execution attestation refused ({version_failure.reason_value}): "
            f"{version_failure.detail}; execution envelope does not carry supported fields "
            f"{', '.join(attest_intoto.SUPPORTED_EXECUTION_PREDICATES)}"
        )
        return None, finding(envelope_path, 0, message, "EXECUTION_ATTESTATION_REFUSED")
    parsed_statement = attest_intoto.parse_statement(payload)
    if parsed_statement.statement is None:
...
def check_execution_attestations(
    ...
    for envelope_path in envelopes:
        ...
        if not isinstance(
            statement.predicate,
            (attest_intoto.ExecutionPredicate, attest_intoto.ExecutionPredicateV2),
        ):
            out.append(
                finding(
                    envelope_path,
                    0,
                    "execution envelope does not carry supported fields "
                    + ", ".join(attest_intoto.SUPPORTED_EXECUTION_PREDICATES),
                    "EXECUTION_ATTESTATION_REFUSED",
                )
            )
            continue
        ...
        if collateral_failed:
            continue
        _, pilot_document, pilot_finding = _parent_resource(root_path, str(pilot.POLICY_PATH))
        if pilot_finding is not None or pilot_document is None:
            out.append(pilot_finding or finding(envelope_path, 0, "pilot policy is unresolved"))
            continue
        parsed_pilot = pilot.parse_pilot_policy(pilot_document)
        if parsed_pilot.policy is None:
            out.append(
                finding(
                    envelope_path, 0, parsed_pilot.outcome.detail, "EXECUTION_ATTESTATION_REFUSED"
                )
            )
            continue
        try:
        ...
        with tempfile.TemporaryDirectory(prefix="trinity-execution-verify-") as temporary:
            ...
            with mock.patch.object(
                ...
                outcome = attest_execution.verify_execution_attestation_evidence(
                    ...
                    harness_config_digest=parsed_pilot.policy.harness_config_digest,
                )
        if not outcome.accepted:
...
HARNESS_CONFIG_APPROVAL_FIELDS: frozenset[str] = frozenset(
    {"approvedBy", "authorizesCompaction", "minMessageChars", "minHeadroomTokens"}
)
HARNESS_CONFIG_APPROVAL_PATHS: dict[str, str] = {
    "requirements": "requirements/harness-approval.json",
    "touchstones": "touchstones/harness-approval.json",
}


def _harness_config_document(
    root_path: Path, relative: Path
) -> tuple[bytes | None, Finding | None]:
    """Read one mirrored half, refusing a redirected or irregular harness path."""

    path = root_path / relative
    symlink = _first_symlink_component(root_path, path)
    if symlink is not None:
        return None, finding(
            symlink,
            0,
            "harness configuration path is a symbolic link",
            harness_config.HarnessConfigFailureReason.HARNESS_CONFIG_MISSING,
        )
    if not path.exists():
        return None, None
    if not path.is_file():
        return None, finding(
            path,
            0,
            "harness configuration is not a regular file",
            harness_config.HarnessConfigFailureReason.HARNESS_CONFIG_MISSING,
        )
    try:
        ...
        return None, finding(
            path,
            0,
            f"harness configuration is unreadable: {exc}",
            harness_config.HarnessConfigFailureReason.HARNESS_CONFIG_MALFORMED,
        )


def _harness_approval(
    path: Path, config: harness_config.HarnessConfig
) -> harness_config.HarnessApproval | None:
    """Resolve one human-owned grant; the root it was read from is its provenance."""

    try:
        document = attest_canonical.parse_json(path.read_bytes())
    except (OSError, attest_canonical.CanonicalizationError):
        return None
    if not isinstance(document, dict) or document.keys() != HARNESS_CONFIG_APPROVAL_FIELDS:
        return None
    authorizes = document["authorizesCompaction"]
    message_chars = document["minMessageChars"]
    headroom = document["minHeadroomTokens"]
    approved_by = document["approvedBy"]
    if (
        not isinstance(authorizes, bool)
        or type(message_chars) is not int
        or type(headroom) is not int
        or not isinstance(approved_by, str)
    ):
        return None
    return harness_config.HarnessApproval(
        authorizes,
        message_chars,
        headroom,
        config.configuration_adequacy.approval_source,
        approved_by,
    )


def check_harness_config(root: str, _evaluation_time: datetime | None = None) -> list[Finding]:
    """Qualify the mirrored harness configuration that a bound digest names.

    FORGE generates "harness/harness-config.json" and mirrors it to
    ".seed/harness-config.json", so both halves must be present and canonically
    identical; a parent carrying neither half declares no harness configuration
    and is exempt rather than refused.

    Adequacy needs an approval the candidate cannot author, so the grant is read
    from the human-write-only root its own approvalSource names: "requirements/
    harness-approval.json" or "touchstones/harness-approval.json", each a closed
    JSON object of exactly approvedBy, authorizesCompaction, minMessageChars, and
    minHeadroomTokens. The root supplies the provenance, so a grant can never be
    relocated into the harness it approves. A configuration present without its
    named carrier, or with a carrier outside that closed shape, is
    HARNESS_CONFIG_UNAPPROVED. Every finding carries the harness-config module's
    own refusal member as its code, so one module owns the vocabulary.
    """

    root_path = Path(root)
    halves = (harness_config.HARNESS_CONFIG_SOURCE, harness_config.HARNESS_CONFIG_PATH)
    documents: list[bytes | None] = []
    for relative in halves:
        document, refusal = _harness_config_document(root_path, relative)
    ...
    if all(document is None for document in documents):
        return []
    out: list[Finding] = []
    configs: list[harness_config.HarnessConfig | None] = []
    for relative, document in zip(halves, documents, strict=True):
        if document is None:
            out.append(
                finding(
                    root_path / relative,
                    0,
                    "harness configuration half is absent; the mirrored pair must be complete",
                    harness_config.HarnessConfigFailureReason.HARNESS_CONFIG_MISSING,
                )
            )
            configs.append(None)
            continue
        result = harness_config.parse_harness_config(document)
        configs.append(result.config)
        if result.outcome.reason is not None:
            out.append(
                finding(
                    root_path / relative,
                    0,
                    f"harness configuration is refused: {result.outcome.detail}",
                    result.outcome.reason,
                )
            )
    if out:
        return out
    source_bytes, mirror_bytes = documents
    source_config, mirror_config = configs
    if source_bytes is None or mirror_bytes is None:
        return out
    if source_config is None or mirror_config is None:
        return out
    mirror = root_path / harness_config.HARNESS_CONFIG_PATH
    binding = harness_config.reconcile(
        harness_config.harness_config_digest(source_bytes),
        harness_config.harness_config_digest(mirror_bytes),
        source_config,
        mirror_config,
    )
    if binding.reason is not None:
        out.append(
            finding(
                mirror,
                0,
                f"mirrored harness configuration diverges from its source: {binding.detail}",
                binding.reason,
            )
        )
    approvals = {
        name: _harness_approval(root_path / relative, mirror_config)
        for name, relative in HARNESS_CONFIG_APPROVAL_PATHS.items()
    }
    adequacy = harness_config.evaluate_adequacy(
        mirror_config, approvals["requirements"], approvals["touchstones"]
    )
    if adequacy.reason is not None:
        out.append(
            finding(
                mirror,
                0,
                f"harness configuration is not approved and adequate: {adequacy.detail}",
                adequacy.reason,
            )
        )
    return out
...
def chain_line_shape(where: str, entry: dict[str, object]) -> list[Finding]:
    ...
    if not isinstance(github_id, str) or not github_id:
        out.append(located_finding(where, "github_id is not a non-empty string"))
    return out
...
def checkpoint_signature_payload(root_path: Path, harness: str, seq: int, head: str) -> bytes:
    value: dict[str, JSONValue] = {
        "audience": display_project_path(root_path, project_root=root_path),
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="trinity-checkpoint-verify-") as temporary:
        ...
        if not verified.verified or verified.principal is None:
            ...
            detail += "; re-sign the checkpoint over the `./` audience"
...
RUN_ID_NAME: re.Pattern[str] = re.compile(r"[a-z0-9][a-z0-9.-]{7,79}\Z")


def _feedback_roots(root_path: Path) -> list[tuple[str, Path]]:
    """Every feedback chain home: each harness root plus each run namespace under it.

    A run's feedback chain lives at ``<harness>/runs/<run>/`` so that two clones never
    append to one chain; every chain is walked under the same rule.
    """
    homes: list[tuple[str, Path]] = []
    for harness in HARNESS_DIRS:
        harness_root = root_path / HARNESS_ROOTS[harness]
        homes.append((harness, harness_root))
        ...
        for run_dir in sorted(runs_dir.iterdir()):
            if (
                ...
                and RUN_ID_NAME.fullmatch(run_dir.name)
            ):
                homes.append((harness, run_dir))
    return homes
...
def check_feedback_chain(
    ...
    root_path = Path(root)
    verifier = SshKeygenBackend() if backend is None else backend
    for harness, harness_root in _feedback_roots(root_path):
...
def check_ledger_checkpoint_chain(
    ...
    for index, record in enumerate(records, start=1):
        ...
        if (
            ...
            continue
        try:
            signature_path = resolve_project_path(
                record["signature"],
                project_root=root_path.resolve(),
                description="CFER checkpoint signature path",
                must_exist=True,
            )
            ...
            out.append(
                finding(
                    log,
                    index,
                    f"CFER checkpoint signature path is invalid: {exc}",
...
def check_bundle_layout(root: str, _evaluation_time: datetime | None = None) -> list[Finding]:
    ...
    for lane_root in BUNDLE_ROOTS:
        ...
        for bundle in bundles:
            ...
            if lane_root in LANE_ROOTS and not (bundle / "trajectories").is_dir():
...
def check_lane_disjoint(root: str, _evaluation_time: datetime | None = None) -> list[Finding]:
    ...
    for uuid, lane_roots in sorted(residency.items()):
        placed_roots = [lane_root for lane_root in lane_roots if lane_root in LANE_ROOTS]
        if len(placed_roots) > 1:
            roots = " and ".join(f'"{lane_root}/"' for lane_root in placed_roots)
            message = f'lane-overlap: bundle uuid "{uuid}" is resident under lane roots {roots}'
            out.append(finding(root_path / placed_roots[0] / uuid, 0, message, "LANE_OVERLAP"))
        if STAGING_ROOT in lane_roots:
            for lane_root in placed_roots:
                message = (
                    f'staging-residue: bundle uuid "{uuid}" is resident under '
                    f"staging/ and {lane_root}/"
                )
                out.append(finding(root_path / STAGING_ROOT / uuid, 0, message, "STAGING_RESIDUE"))
    return out
...
def _ship_bundles(bundles: list[Path]) -> tuple[set[Path], list[Finding]]:
    ...
    for bundle in bundles:
        if bundle.parent.name in LANE_ROOTS:
...
def _false_exemption_declarations(document: str, bundle_name: str) -> set[str]:
    ...
    sections: list[str] = []
    for index, (offset, uuid) in enumerate(starts):
        if uuid != bundle_name:
            continue
        end = starts[index + 1][0] if index + 1 < len(starts) else len(document)
        sections.append(document[offset:end])
    if not sections:
        sections.append(document)
    declarations: set[str] = set()
    for field in FALSE_EXEMPTION_FIELDS:
        # Every repeated atom below matches a line exactly one way, so a failing
        # search stays linear in the section length; the original `[ \t]+.*`
        # form backtracked exponentially over an all-indented YAML block. The
        # captured indent bounds the block to the field's own nested lines.
        pattern = (
            rf"(?m)^([ \t]*){field}[ \t]*:[ \t]*(?:#[^\n]*)?\n"
            rf"(?:^\1[ \t][^\n]*\n)*?^\1[ \t]+present[ \t]*:[ \t]*false[ \t]*(?:#[^\n]*)?$"
        )
        if any(re.search(pattern, section) for section in sections):
...
QUALIFICATION_CHECKS: list[ParentCheck] = [
    check_parent_tree,
    check_front_doors,
    ...
    # Delivery-integrity and anti-sabotage families landed after the 2026-09-07
    # sample delivery; each module owns its refusal vocabulary and fixtures.
    *[check for _name, check in HARBOR_CHECKS],
    *[check for _name, check in PIPELINE_CHECKS],
    check_epochs,
    *[check for _name, check in SABOTAGE_CHECKS if check.__name__ != "check_disposition_binding"],
    *[check for _name, check in SENTINEL_CHECKS],
    *[check for name, check in GATE_CHECKS if name == "check_gates_installed"],
]

FINAL_AUTHORIZATION_CHECKS: list[ParentCheck] = [
    check for _name, check in SABOTAGE_CHECKS if check.__name__ == "check_disposition_binding"
]
FINAL_GATE_CHECKS: list[ParentCheck] = [
    check for name, check in GATE_CHECKS if name != "check_gates_installed"
]

PARENT_CHECKS: list[ParentCheck] = [
    *QUALIFICATION_CHECKS,
    *FINAL_AUTHORIZATION_CHECKS,
    *[check for _name, check in RELEASE_CHECKS],
    *FINAL_GATE_CHECKS,
]
...
def run_qualification(root: str, evaluation_time: datetime | None = None) -> list[Finding]:
    """Run substantive candidate checks without final release authorization prerequisites."""

    if not Path(root).is_dir():
        return [finding(root, 0, "parent project root is not a directory")]
    instant = gate_evaluation_time() if evaluation_time is None else evaluation_time
    findings: list[Finding] = []
    for fn in QUALIFICATION_CHECKS:
        findings += fn(root, instant)
    return findings
...
def public_findings(findings: list[Finding], *, project_root: Path) -> list[Finding]:
    return [
        Finding(
            item.code,
            item.severity,
...
def main(argv: list[str]) -> int:
    ...
    if mode == "parent":
        ...
        try:
            ...
            parent_root = resolve_project_path(
                argv[2],
            ...
            print(f"integrity refused: {exc}", file=sys.stderr)
            return 2
        evaluation_time = gate_evaluation_time()
        print(f"evaluated_at: {format_evaluation_time(evaluation_time)}")
        return emit(
            public_findings(
                _run_parent_at(str(parent_root), evaluation_time), project_root=project_root
            )
        )
    if mode == "json":
        if len(argv) < PARENT_ARGUMENTS:
