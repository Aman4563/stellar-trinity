...
BITE = ("tests", "bite")
USAGE = (
    "usage: citations.py [check|update] [--root PATH]\n"
    "       citations.py rekey <bite-file>.md:<line> ... [--root PATH]"
)
...
def rekey(corpus: Corpus, targets: list[str]) -> list[str]:
    anchors = corpus.load_lock()
    problems: list[str] = []
    for target in targets:
        name, _, raw = target.rpartition(":")
        source = corpus.bite / name
        if not name or not raw.isdigit() or not source.is_file():
            problems.append(f"{target}: expected <bite-file>.md:<line>")
            continue
        dropped = [
            anchor_key(cited)
            for cited in citations_in(source, corpus.bite)
            if cited.line == int(raw) and anchor_key(cited) in anchors
        ]
        if not dropped:
            problems.append(f"{target}: no recorded anchor to drop")
            continue
        for key in dropped:
            del anchors[key]
        print(f"rekeyed {target}: dropped {len(dropped)} anchor(s)")
    corpus.write_lock(anchors)
    return problems
...
def main(argv: list[str]) -> int:
    ...
    if mode == "rekey":
        targets = positionals(argv)
        if not targets:
            print(USAGE)
            return 2
        problems = rekey(corpus, targets)
        if problems:
            print("\n".join(problems))
        return 1 if problems else 0
    print(USAGE)
