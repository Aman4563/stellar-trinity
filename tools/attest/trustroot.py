...
class TrustRoot:
    ...
    def authorize(
        ...
        *,
        ...
        excluded_principals: Collection[str] = (),
    ) -> AttestationOutcome:
        ...
        for principal in excluded_principals:
            if not isinstance(principal, str) or not principal:
                return _malformed_outcome("excluded principals must be non-empty strings")
            excluded.add(principal)
        if producer_principal is not None:
            excluded.add(producer_principal)
        excluded_were_eligible = eligible.intersection(excluded)
        eligible.difference_update(excluded)

        if len(eligible) >= role.threshold:
            return AttestationOutcome.accept(tuple(sorted(eligible)))
        if excluded_were_eligible:
            if producer_principal is not None and excluded_were_eligible == {producer_principal}:
                detail = f'producer "{producer_principal}" cannot approve its own artifact'
            else:
                detail = "producer signers cannot contribute to approval: " + ", ".join(
                    sorted(excluded_were_eligible)
                )
            return AttestationOutcome.refuse(
