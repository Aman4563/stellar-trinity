...
def _policy_path(document: dict[str, JSONValue], name: str, authority_root: Path) -> Path:
    try:
        return resolve_project_path(
            _policy_text(document, name),
            project_root=authority_root,
            description=f"gate policy field {name!r}",
            must_exist=name == "projectRoot",
        )
...
def load_gate_policy(path: Path, *, authority_root: Path) -> GatePolicy:
    ...
    project_root = _policy_path(document, "projectRoot", authority_root)
    replay_store = _policy_path(document, "replayStore", authority_root)
