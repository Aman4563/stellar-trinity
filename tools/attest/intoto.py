...
if TYPE_CHECKING:
    from tools import harness_config
else:
    try:
        from tools import harness_config
...
# allow: SIZE_OK - Preserve the historical v1 parser verbatim; v2 lives in execution_v2.py.
ExecutionPredicateV2 = execution_v2.ExecutionPredicateV2
RolloutTelemetry = execution_v2.RolloutTelemetry

STATEMENT_TYPE = "https://in-toto.io/Statement/v1"
APPROVAL_PREDICATE_TYPE = "trinity.approval/v1"
EXECUTION_PREDICATE_TYPE = "trinity.execution/v1"
EXECUTION_PREDICATE_TYPE_V2 = execution_v2.EXECUTION_PREDICATE_TYPE_V2
SUPPORTED_EXECUTION_PREDICATES = (EXECUTION_PREDICATE_TYPE, EXECUTION_PREDICATE_TYPE_V2)
RELEASE_EXECUTION_PREDICATE = EXECUTION_PREDICATE_TYPE_V2
...
Predicate: TypeAlias = ApprovalPredicate | ExecutionPredicate | ExecutionPredicateV2 | CferPredicate
...
def parse_statement(value: JSONValue) -> StatementParseResult:
    ...
    elif predicate_type == EXECUTION_PREDICATE_TYPE_V2:
        predicate_result = execution_v2.parse_execution_v2(predicate_value)
...
def _required_collateral(statement: StatementV1) -> tuple[ResourceDescriptor, ...]:
    ...
    if isinstance(predicate, CferPredicate):
        return ()
    assert isinstance(predicate, (ExecutionPredicate, ExecutionPredicateV2))
    commitment_resource = (
    ...
    if isinstance(predicate, ExecutionPredicateV2):
        resources.extend(
            (
...
def validate_collateral(
    ...
    """Bind immutable collateral; v2 harness_config uses trinity.jcs-lf.v1, others raw bytes."""

    required = required_collateral(statement)
    predicate = statement.predicate
    trajectory_manifest_name = (
        predicate.population_sequence_commitment.name
        if isinstance(predicate, (ExecutionPredicate, ExecutionPredicateV2))
    ...
    for resource in required:
        ...
        if isinstance(predicate, ExecutionPredicateV2) and resource is predicate.harness_config:
            try:
                digest = harness_config.harness_config_digest(content)
            except (ValueError, RecursionError, UnicodeError):
                return _refuse(
                    AttestationFailureReason.SUBJECT_DIGEST_MISMATCH,
                    "harness config collateral is malformed",
                )
        if digest != resource.digest.sha256:
            return _refuse(
                AttestationFailureReason.SUBJECT_DIGEST_MISMATCH,
...
def required_role_for_predicate_type(predicate_type: str) -> str | None:
    ...
    if predicate_type in SUPPORTED_EXECUTION_PREDICATES:
...
def validate_claimed_identity(
    ...
    elif isinstance(predicate, (ExecutionPredicate, ExecutionPredicateV2)):
