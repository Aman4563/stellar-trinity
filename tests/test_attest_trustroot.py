...
def test_every_producer_signer_is_excluded_from_group_approval(tmp_path: Path) -> None:
    roles: list[JSONValue] = [
        {"name": "gate_producer", "threshold": 2},
        {"name": "gate_approver", "threshold": 1},
    ]
    principals: list[JSONValue] = [
        {"name": ACTIVE, "roles": ["gate_producer", "gate_approver"]},
        {"name": SECOND, "roles": ["gate_producer"]},
        {"name": THIRD, "roles": ["gate_approver"]},
    ]
    root = write_root(tmp_path, roles=roles, principals=principals, revocations=[])
    producers = query(root, "gate_producer", [ACTIVE, SECOND, THIRD])
    assert producers == AttestationOutcome.accept(tuple(sorted((ACTIVE, SECOND))))
    refused = authorize(
        root,
        role_name="gate_approver",
        verified_principals=[ACTIVE, SECOND],
        evaluation_time=EVALUATION,
        trusted_version=VERSION,
        excluded_principals=producers.principals,
    )
    assert_refusal(refused, AttestationFailureReason.SELF_APPROVAL)
    accepted = authorize(
        root,
        role_name="gate_approver",
        verified_principals=[ACTIVE, SECOND, THIRD],
        evaluation_time=EVALUATION,
        trusted_version=VERSION,
        excluded_principals=producers.principals,
    )
    assert accepted == AttestationOutcome.accept((THIRD,))
