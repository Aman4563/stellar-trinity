...
def write_clean_contracts(root: Path) -> list[str]:
    ...
    content = {
        "ENGRAM.md": "DIRECTIVE.md HARDNESS.md TRACKING.md "
...
def test_integrity_parent_json_dispatch_pair(tmp_path: Path) -> None:
    ...
    clean = run_cli(str(INTEGRITY), "json", "parent", "./clean", evaluation_time, cwd=tmp_path)
    planted = run_cli(str(INTEGRITY), "json", "parent", "./planted", evaluation_time, cwd=tmp_path)
    clean_rows = json.loads(clean.stdout)
