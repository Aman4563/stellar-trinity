# allow: SIZE_OK - Task 18 requires this regression matrix here with existing fixtures unchanged.
...
from tools.attest.execution_v2 import TerminationReason
...
def valid_execution_v2() -> dict[str, JSONValue]:
    statement = valid_execution()
    statement["predicateType"] = "trinity.execution/v2"
    predicate = statement["predicate"]
    assert isinstance(predicate, dict)
    predicate.update({name: descriptor(name, name.encode()) for name in V2_DESCRIPTORS})
    predicate["rolloutDurations"] = [
        {"rollout": "rollout-1", "seconds": 12.5},
        {"rollout": "rollout-2", "seconds": 0},
    ]
...
def test_v2_predicate_parses_with_five_new_fields() -> None:
    # Given a complete v2 execution statement.
    statement = valid_execution_v2()
    # When its signed structure is parsed.
    result = parse_statement(statement)
    # Then all five fidelity fields survive as typed values.
    ...
    assert (
        tuple(
            resource.name
            for resource in (
                predicate.harness_config,
                predicate.effective_config_reconciliation,
                predicate.population_roster,
                predicate.observation_capture,
            )
        )
        == V2_DESCRIPTORS
    )
    assert predicate.rollout_telemetry == tuple(
        intoto.RolloutTelemetry(rollout, 3, 0, TerminationReason.COMPLETED, 0)
        for rollout in ("rollout-1", "rollout-2")
    )


@pytest.mark.parametrize("field", V2_FIELDS)
def test_v2_refuses_unknown_field(field: str) -> None:
    # Given an extra field beside each of the fifteen required fields.
    statement = valid_execution_v2()
    predicate = statement["predicate"]
    assert isinstance(predicate, dict)
    predicate[f"{field}Unknown"] = predicate[field]
    # When parsed.
    result = parse_statement(statement)
    # Then the v2 closed field set refuses it.
    assert_refused(result.outcome, AttestationFailureReason.PREDICATE_MALFORMED)
    assert "trinity.execution/v2" in result.outcome.detail


@pytest.mark.parametrize("field", V2_FIELDS)
def test_v2_refuses_absent_field(field: str) -> None:
    # Given each required field omitted, including replay.
    statement = valid_execution_v2()
    predicate = statement["predicate"]
    assert isinstance(predicate, dict)
    del predicate[field]
...
@pytest.mark.parametrize(
    ...
    [
        ("rollout-1",),
        ("rollout-1", "rollout-2", "extra"),
        ("rollout-1", "rollout-2", "rollout-1"),
    ],
    ids=["missing", "extra", "duplicate"],
)
def test_v2_refuses_telemetry_population_mismatch(ids: tuple[str, ...]) -> None:
    # Given telemetry describing a different population from the durations.
    statement = valid_execution_v2()
    predicate = statement["predicate"]
    assert isinstance(predicate, dict)
    predicate["rolloutTelemetry"] = [
    ...
    # When parsed.
    result = parse_statement(statement)
    # Then the population mismatch is refused with the affected identifier.
    assert_refused(result.outcome, AttestationFailureReason.PREDICATE_MALFORMED)
    affected = "rollout-2" if len(ids) == 1 else ids[-1]
    assert affected in result.outcome.detail


def test_v2_refuses_unknown_termination_reason() -> None:
    # Given an unregistered termination reason.
    statement = valid_execution_v2()
    predicate = statement["predicate"]
    assert isinstance(predicate, dict)
    telemetry = predicate["rolloutTelemetry"]
    assert isinstance(telemetry, list)
    entry = telemetry[0]
    assert isinstance(entry, dict)
    entry["terminationReason"] = "operator_says_clean"
    # When parsed.
    result = parse_statement(statement)
    # Then vocabulary expansion fails closed.
    assert_refused(result.outcome, AttestationFailureReason.PREDICATE_MALFORMED)


def test_v2_requires_non_empty_telemetry() -> None:
    # Given no rollout telemetry.
    statement = valid_execution_v2()
    predicate = statement["predicate"]
    assert isinstance(predicate, dict)
    predicate["rolloutTelemetry"] = []
    # When parsed.
    result = parse_statement(statement)
    # Then an empty population is malformed.
    assert_refused(result.outcome, AttestationFailureReason.PREDICATE_MALFORMED)


def test_v1_predicate_still_parses_unchanged() -> None:
    # Given the original fixture and the pre-change parse-result byte fingerprint.
    expected = "dd0d7083809ae5e7369cd0791b857fbbf9bb07c25d07366cca8018cae8277c9a"
    # When the unchanged fixture is parsed.
    result = parse_statement(valid_execution())
    # Then every parsed field, type, and outcome remains byte-identical.
    assert hashlib.sha256(repr(result).encode()).hexdigest() == expected


def test_release_predicate_constant_is_v2() -> None:
    # Given registration of historical and current execution formats.
    # When consumers select the release predicate.
    release = getattr(intoto, "RELEASE_EXECUTION_PREDICATE", None)
    # Then release selects v2, not the historical format.
    assert release == "trinity.execution/v2"
    assert release == intoto.EXECUTION_PREDICATE_TYPE_V2
    assert (EXECUTION_PREDICATE_TYPE, release) == intoto.SUPPORTED_EXECUTION_PREDICATES


def test_required_collateral_covers_new_descriptors() -> None:
    # Given equivalent historical and fidelity-bearing statements.
    v1 = parse_statement(valid_execution())
    v2 = parse_statement(valid_execution_v2())
    assert v1.statement is not None
    assert v2.statement is not None, v2.outcome
    # When collateral requirements are enumerated.
    historical = intoto.required_collateral(v1.statement)
    current = intoto.required_collateral(v2.statement)
    # Then v1 remains unchanged and v2 adds exactly the four new descriptors.
    assert {resource.name for resource in historical} == set(EXECUTION_COLLATERAL)
    assert current[: len(historical)] == historical
    assert {resource.name for resource in current} == set(EXECUTION_COLLATERAL) | set(
        V2_DESCRIPTORS
    )
