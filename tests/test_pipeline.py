...
def test_reconcile_skips_uninitialised_lane_submodule_checkouts(tmp_path: Path) -> None:
    """CI checks the parent out with submodules disabled; absence is not drift.

    A lane that is a declared submodule and is not checked out is an existing but
    empty directory. Reading its missing README as reconcile drift would refuse
    every continuous integration run on a parent whose lanes are submodules. An
    empty lane that is not a declared submodule still owes a rendered README.
    """

    lane_project(tmp_path, 1)
    pipeline.reconcile(tmp_path)
    assert pipeline.reconcile(tmp_path, check=True).findings == ()

    for lane in ("samples", "delivery"):
        directory = tmp_path / lane
        for child in list(directory.iterdir()):
            shutil.rmtree(child) if child.is_dir() else child.unlink()
        assert directory.is_dir() and not any(directory.iterdir())

    # Without a .gitmodules declaration the empty lanes still owe a README.
    undeclared = [item.message for item in pipeline.reconcile(tmp_path, check=True).findings]
    assert "samples/README.md is not rendered" in undeclared
    assert "delivery/README.md is not rendered" in undeclared
    ...
    declared = [item.message for item in pipeline.reconcile(tmp_path, check=True).findings]
    assert "samples/README.md is not rendered" not in declared
    assert "delivery/README.md is not rendered" not in declared
