...
@bite("forge.md:F21")
def test_parent_rejects_missing_samples_and_delivery(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    build_parent(root)
    (root / "samples").rmdir()
    (root / "delivery").rmdir()
...
@pytest.mark.parametrize("lane_root", ["samples", "delivery"])
@bite("forge.md:F32")
def test_placed_lane_bundle_without_trajectories_is_refused(tmp_path: Path, lane_root: str) -> None:
    root = tmp_path / "parent"
    bundle = build_bundle(root, BUNDLE_UUID, lane_root=lane_root)
    (bundle / "trajectories").rmdir()

    findings = integrity.check_bundle_layout(str(root))

    assert any(
        'missing required rollout evidence directory "trajectories/"' in item.message
