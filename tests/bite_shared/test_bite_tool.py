...
BACKTICKED_AS_THE_REAL_README_WRITES_IT = "`shared.md`"
...
def build_restatements(
    ...
    *,
    ...
    (root / "tests" / "bite" / "README.md").write_text(
        "| File | Scope | Tests |\n"
        "| --- | --- | --- |\n"
        f"| {BACKTICKED_AS_THE_REAL_README_WRITES_IT} | Shared doctrine. | {readme_files} |\n"
        "\n"
        "| Measure | Value |\n"
        "| --- | --- |\n"
        "| Total tests | 1 |\n"
...
def test_a_drifted_backticked_file_row_is_a_named_finding(tmp_path: Path) -> None:
    build_corpus(tmp_path, registered=True)
    build_restatements(tmp_path, readme_files=9)
    result = run_bite(tmp_path)
    assert result.returncode == 1
    assert "bite rows in shared.md restated as 9 but the corpus holds 1" in result.stdout


def test_a_matching_backticked_file_row_stays_clean(tmp_path: Path) -> None:
    build_corpus(tmp_path, registered=True)
    build_restatements(tmp_path, readme_files=1)
    result = run_bite(tmp_path)
    assert result.returncode == 0
    assert "bite rows in" not in result.stdout
