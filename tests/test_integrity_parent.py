# allow: SIZE_OK - Shared parent-gate suite; Task 19 changes execution fixtures only.
...
from tools.project_paths import display_project_path

from tests.attest_execution_fixtures import FIDELITY_COLLATERAL, HARNESS_DIGEST, with_fidelity
...
from tests.test_pilot import policy_document
...
TRAJECTORY_ROOT = "d00cdef0a7b38cce6b29a75b09e4ae7c02d6af4a24f5d44fa4379054230105cb"
ROLLOUT_ONE = Path("samples/6ba7b810-9dad-51d1-80b4-00c04fd430c8/trajectories/rollout-1.json")
    ...
    return {
        ...
        "predicate": {
            ...
            "replay": {
                ...
                "audience": display_project_path(project_root.resolve(), project_root=Path.cwd()),
                "sequence": 1,
...
def write_parent_trust(root: Path, document: dict[str, JSONValue]) -> None:
    ...
    authority_root = Path.cwd().resolve()
    project_value = display_project_path(root.resolve(), project_root=authority_root)
    policy_path = integrity.approval_gate_policy_path(root, authority_root=authority_root)
    policy_path.parent.mkdir(parents=True, exist_ok=True)
    replay_name = hashlib.sha256(project_value.encode()).hexdigest() + ".jsonl"
    replay_store = authority_root / ".trinity" / "state" / replay_name
    policy_path.write_bytes(
        canonicalize(
            {
                "expectedAudience": project_value,
                "projectRoot": project_value,
                "replayStore": "./" + replay_store.relative_to(authority_root).as_posix(),
...
@pytest.mark.parametrize("relative", ["./parent", "./"])
def test_approval_policy_is_keyed_by_canonical_relative_root(tmp_path: Path, relative: str) -> None:
    root = tmp_path / relative
    key = hashlib.sha256(relative.encode()).hexdigest()

    policy_path = integrity.approval_gate_policy_path(root, authority_root=tmp_path)

    assert policy_path == tmp_path / ".trinity" / "parents" / f"{key}.json"


def test_approval_policy_with_absolute_audience_is_refused_with_remedy(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    build_parent(root)
    write_approval_gate(root)
    policy_path = integrity.approval_gate_policy_path(root)
    document = parse_json(policy_path.read_bytes())
    assert isinstance(document, dict)
    document["expectedAudience"] = str(root.resolve())
    policy_path.write_bytes(canonicalize(document))

    findings = integrity.check_approval_envelopes(str(root), EVALUATION_TIME, StubBackend())

    assert any(
        item.code == "APPROVAL_TRUST_ROOT_MALFORMED"
        and "expectedAudience" in item.message
        and "./" in item.message
...
def test_approval_replay_is_refused_in_a_separate_gate_invocation(tmp_path: Path) -> None:
    ...
    first = subprocess.run(command, cwd=root.parent, capture_output=True, text=True, check=False)
    second = subprocess.run(command, cwd=root.parent, capture_output=True, text=True, check=False)
...
def test_parent_cli_records_gate_time_and_rejects_a_presenter_supplied_instant(
    ...
    root = tmp_path / "parent"
    build_parent(root)
    command = [sys.executable, str(ROOT / "tools" / "integrity.py"), "parent", "./parent"]

    ordinary = subprocess.run(command, cwd=root.parent, capture_output=True, text=True, check=False)
...
def write_execution_attestation(
    ...
    *,
    ...
    resources = {
        ...
        "scorer": b"scorer\n",
        "population": (TRAJECTORY_FIXTURE / TRAJECTORY_MANIFEST_NAME).read_bytes(),
        "decoy": b"decoy\n",
        "schedule": b"schedule\n",
        "budget": b"budget\n",
        **FIDELITY_COLLATERAL,
    ...
    if trajectory_profile:
        shutil.copytree(
            TRAJECTORY_FIXTURE / "deliverables" / "task" / "trajectories",
            root / "samples" / BUNDLE_UUID / "trajectories",
            dirs_exist_ok=True,
        )
        ...
        ).replace(b"./deliverables/task/", f"./samples/{BUNDLE_UUID}/".encode())
    ...
    statement: dict[str, JSONValue] = {
        ...
        "predicate": {
            ...
            "replay": {
                ...
                "audience": display_project_path(root.resolve(), project_root=Path.cwd()),
                "sequence": 1,
                "runId": f"run-{root.name}",
            },
        },
    }
    policy_value = json.loads(policy_document())
    policy_value["harnessConfigDigest"] = HARNESS_DIGEST
    policy_path = root / ".seed/pilot-policy.json"
    policy_path.parent.mkdir(exist_ok=True)
    policy_path.write_text(json.dumps(policy_value), encoding="utf-8")
    envelope = Envelope(
        payload_type=IN_TOTO_PAYLOAD_TYPE,
        payload=canonicalize(with_fidelity(statement)),
...
def test_signed_v2_execution_fixture_through_parent_gate(tmp_path: Path, case: str) -> None:
    # Given a frozen envelope signed with the existing test-only Ed25519 key.
    root = tmp_path / case
    root.mkdir()
    write_parent_trust(
        root,
        trust_root(
            principal="valid-signer@trinity.test",
            principal_roles=("execution_operator",),
        ),
    )
    stub = write_execution_attestation(root)
    document = (ATTEST_FIXTURES / "execution-v2" / case / "execution.dsse").read_bytes()
    parsed = parse_envelope(document)
    assert parsed.envelope is not None
    statement = parse_json(parsed.envelope.payload)
    assert isinstance(statement, dict)
    predicate = statement["predicate"]
    assert isinstance(predicate, dict)
    replay = predicate["replay"]
    assert isinstance(replay, dict)
    replay["audience"] = display_project_path(root.resolve(), project_root=Path.cwd())
    if case == "clean":
        # The frozen bytes predate the canonical harness-config interface; bind the
        # canonical collateral digest before re-signing so only the planted case diverges.
        harness_descriptor = predicate["harnessConfig"]
        assert isinstance(harness_descriptor, dict)
        harness_descriptor["digest"] = {"sha256": HARNESS_DIGEST}
    key = tmp_path / "TEST_ONLY_key"
    shutil.copyfile(ATTEST_FIXTURES / "TEST_ONLY_valid_ed25519", key)
    key.chmod(0o600)
    ...
    stub.unlink()
    (stub.parent / f"{hashlib.sha256(document).hexdigest()}.dsse").write_bytes(document)
    shutil.copyfile(ATTEST_FIXTURES / "allowed_signers", root / ".memory/allowed_signers")
    # When the production parent gate verifies signatures, collateral and the policy pin.
    findings = integrity.check_execution_attestations(str(root), EVALUATION_TIME)
    # Then only the planted divergent config is refused.
    if case == "clean":
        assert findings == []
    else:
        assert any("execution_harness_config_divergent" in item.message for item in findings)


@pytest.mark.skipif(shutil.which("ssh-keygen") is None, reason="ssh-keygen is required")
def test_signed_execution_attestation_verifies_after_the_parent_moves(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    authority = tmp_path / "authority"
    root = authority / "parent"
    root.mkdir(parents=True)
    monkeypatch.chdir(authority)
    write_parent_trust(
        root,
        trust_root(principal="valid-signer@trinity.test", principal_roles=("execution_operator",)),
    )
    stub = write_execution_attestation(root)
    parsed = parse_envelope(stub.read_bytes())
    assert parsed.envelope is not None
    statement = parse_json(parsed.envelope.payload)
    assert isinstance(statement, dict)
    predicate = statement["predicate"]
    assert isinstance(predicate, dict)
    predicate["operatorIdentity"] = "valid-signer@trinity.test"
    key = authority / "TEST_ONLY_key"
    shutil.copyfile(ATTEST_FIXTURES / "TEST_ONLY_valid_ed25519", key)
    key.chmod(0o600)
    document = sign_envelope(
        SshKeygenBackend(), IN_TOTO_PAYLOAD_TYPE, canonicalize(statement), key_path=key
    ).to_json()
    stub.unlink()
    (stub.parent / f"{hashlib.sha256(document).hexdigest()}.dsse").write_bytes(document)
    shutil.copyfile(ATTEST_FIXTURES / "allowed_signers", root / ".memory/allowed_signers")
    elsewhere = tmp_path / "elsewhere"
    shutil.copytree(authority, elsewhere)
    before = integrity.check_execution_attestations(str(root), EVALUATION_TIME)

    monkeypatch.chdir(elsewhere)
    after = integrity.check_execution_attestations(str(elsewhere / "parent"), EVALUATION_TIME)
...
def test_front_door_rosters_match_the_contracts() -> None:
    ...
    assert integrity.FRONT_DOOR_CONTRACTS[".opencode/commands/harness.md"] == "FORGE.md"
    assert integrity.FRONT_DOOR_CONTRACTS[".agents/skills/rig/SKILL.md"] == "FORGE.md"
    assert integrity.FRONT_DOOR_CONTRACTS[".opencode/commands/genesis.md"] == "ENGRAM.md"
    templates = {
        f".opencode/commands/{path.name}"
        for path in integrity.DOOR_TEMPLATES_DIR.glob("commands/*.md")
    } | {
        f".agents/skills/{path.parent.name}/SKILL.md"
        for path in integrity.DOOR_TEMPLATES_DIR.glob("skills/*/SKILL.md")
    }
    assert templates == set(integrity.FRONT_DOOR_CONTRACTS)
    for relative, contract in integrity.FRONT_DOOR_CONTRACTS.items():
        text = integrity.door_template_path(relative).read_text(encoding="utf-8")
        body = integrity._front_door_body(text)
        assert contract in body, relative
        assert len(body.encode("utf-8")) <= integrity.FRONT_DOOR_BODY_MAX_BYTES, relative
        instrument = contract.removesuffix(".md")
        if instrument != "MAESTRO":
            for moment in ("preflight", "report"):
                assert f"parent-gate instrument={instrument} moment={moment}" in body, relative


def test_the_ingest_door_accepts_a_source_class_qualified_identifier() -> None:
    # Given the ingest command template and its installed fixture copies.
    template = integrity.DOOR_TEMPLATES_DIR / "commands" / "ingest.md"

    # When the command's routing description is read from its frontmatter.
    ...
    # Then routing accepts qualified sources and both fixtures match the template.
    assert "source-class-qualified identifier" in description
    assert "bare arXiv identifier" not in description
    for fixture in ("clean", "planted"):
        planted = FRONT_DOOR_FIXTURES / fixture / ".opencode" / "commands" / "ingest.md"
        assert planted.read_bytes() == body.encode("utf-8")
...
def test_front_doors_reject_planted_fixture_and_accept_clean_fixture(tmp_path: Path) -> None:
    ...
    drifted = {
        finding.message.split('"')[1] for finding in findings if finding.code == "FRONT_DOOR_DRIFT"
    }
    assert drifted == {".opencode/commands/forge.md", ".opencode/commands/hardness.md"}
...
@pytest.mark.parametrize("ledger", [False, True], ids=["feedback", "ledger"])
def test_checkpoint_signature_payload_audience_is_project_relative(
    tmp_path: Path, ledger: bool
) -> None:
    root = tmp_path / "parent"
    record = {
        "prior_root": integrity.CHAIN_GENESIS,
        "new_root": "a" * 64,
        "leaf_count": "1",
    ...
    payload = (
        integrity.ledger_checkpoint_signature_payload(root, record)
        if ledger
        else integrity.checkpoint_signature_payload(root, "memory", 1, "a" * 64)
    )

    assert b'"audience":"./"' in payload
    assert str(root.resolve()).encode() not in payload


@pytest.mark.skipif(shutil.which("ssh-keygen") is None, reason="ssh-keygen is required")
@pytest.mark.parametrize("ledger", [False, True], ids=["feedback", "ledger"])
def test_checkpoint_signature_verifies_after_parent_moves(tmp_path: Path, ledger: bool) -> None:
    root = tmp_path / "parent"
    build_parent(root)
    entries = build_chain()
    ...
    if ledger:
        ...
        (root / ".memory/ledger.yaml").write_text("- lever_id: lever-1\n", encoding="utf-8")
        (root / ".memory/checkpoints.yaml").write_text(
            "- " + "\n  ".join(f"{key}: {value}" for key, value in record.items()) + "\n",
            encoding="utf-8",
        )
        signature = SshKeygenBackend(timeout_seconds=5.0).sign(
            integrity.ledger_checkpoint_signature_payload(root, record),
            key_path=root / "TEST_ONLY_valid_ed25519",
        )
        (root / ".memory/ledger-head.sig").write_bytes(signature)
    check = integrity.check_ledger_checkpoint_chain if ledger else integrity.check_feedback_chain
    assert check(str(root), EVALUATION_TIME) == []

    moved = shutil.copytree(root, tmp_path / "moved")
    findings = check(str(moved), EVALUATION_TIME)
...
@bite("shared.md:A63")
def test_feedback_chain_walks_every_run_namespace(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    build_parent(root)
    run_dir = root / ".memory" / "runs" / "engram-ada-20260917t120000z-abcdef"
    run_dir.mkdir(parents=True)
    (run_dir / integrity.FEEDBACK_CHAIN).write_text("", encoding="utf-8")
    stray = root / ".memory" / "runs" / "not a run"
    stray.mkdir()
    (stray / integrity.FEEDBACK_CHAIN).write_text("", encoding="utf-8")

    findings = integrity.check_feedback_chain(str(root), EVALUATION_TIME, StubBackend())

    assert [Path(item.path) for item in findings] == [run_dir / integrity.FEEDBACK_CHAIN]
    assert findings[0].code == "FEEDBACK_CHAIN_EMPTY"
...
def test_feedback_checkpoint_absolute_audience_requires_resigning(tmp_path: Path) -> None:
    ...
    signature = SshKeygenBackend(timeout_seconds=5.0).sign(
        canonicalize(payload), key_path=source / "TEST_ONLY_valid_ed25519"
    )
    (source / ".memory/feedback-head.sig").write_bytes(signature)

    findings = integrity.check_feedback_chain(str(source), EVALUATION_TIME)

    assert any(finding.code == "CHECKPOINT_SIGNATURE_INVALID" for finding in findings)
    assert any("re-sign the checkpoint over the `./` audience" in item.message for item in findings)
...
@pytest.mark.parametrize("lane_root", integrity.BUNDLE_ROOTS)
...
def test_false_exemption_declarations_stay_linear_on_indented_section_without_false() -> None:
    document = f"tasks:\n  - uuid: {BUNDLE_UUID}\n    verifier_robustness:\n      present: true\n"
    document += "".join(f"      key_{index}: value_{index}\n" for index in range(2000))
    started = time.perf_counter()
    declarations = integrity._false_exemption_declarations(document, BUNDLE_UUID)
    assert time.perf_counter() - started < 2.0
    assert declarations == set()


def test_false_exemption_declarations_scan_every_uuid_occurrence() -> None:
    other = "1b21d4ac-055a-52e7-b754-0620f337b0d4"
    document = (
        f"manifest:\n  - {BUNDLE_UUID}\n  - {other}\n"
        f"tasks:\n  - uuid: {other}\n    reference_emulation:\n      present: true\n"
        f"  - uuid: {BUNDLE_UUID}\n    reference_emulation:\n      present: false\n"
    ...
    assert integrity._false_exemption_declarations(document, BUNDLE_UUID) == {"reference_emulation"}
    assert integrity._false_exemption_declarations(document, other) == set()


def test_false_exemption_declarations_do_not_read_a_sibling_field() -> None:
    document = (
        f"  - uuid: {BUNDLE_UUID}\n    reference_emulation:\n      present: true\n"
        "    verifier_robustness:\n      present: false\n"
    )
    assert integrity._false_exemption_declarations(document, BUNDLE_UUID) == {"verifier_robustness"}
...
def test_stale_deliverables_digest_key_is_ignored(tmp_path: Path) -> None:
    carrier = tmp_path / ".memory" / "ledger.yaml"
    carrier.parent.mkdir()
    carrier.write_text(f"deliverables_digest: {'0' * 64}\n", encoding="utf-8")

    findings = integrity.check_work_differential_digests(str(tmp_path))

    assert findings == []


@pytest.mark.parametrize("lane_root", integrity.LANE_ROOTS)
def test_placed_bundle_without_trajectories_is_refused(tmp_path: Path, lane_root: str) -> None:
    bundle = build_bundle(tmp_path, BUNDLE_UUID, lane_root=lane_root)
    shutil.rmtree(bundle / "trajectories", ignore_errors=True)

    findings = integrity.check_bundle_layout(str(tmp_path))

    assert [item.code for item in findings] == ["INTEGRITY_VIOLATION"]
    assert 'missing required rollout evidence directory "trajectories/"' in findings[0].message


@pytest.mark.parametrize("lane_root", integrity.LANE_ROOTS)
def test_placed_bundle_with_empty_trajectories_directory_passes(
    tmp_path: Path, lane_root: str
) -> None:
    bundle = build_bundle(tmp_path, BUNDLE_UUID, lane_root=lane_root)
    (bundle / "trajectories").mkdir(exist_ok=True)
...
@pytest.mark.parametrize("lane_root", integrity.LANE_ROOTS)
def test_placed_bundle_with_trajectories_file_is_refused(tmp_path: Path, lane_root: str) -> None:
    bundle = build_bundle(tmp_path, BUNDLE_UUID, lane_root=lane_root)
    shutil.rmtree(bundle / "trajectories", ignore_errors=True)
    (bundle / "trajectories").write_text("not a directory", encoding="utf-8")
...
@pytest.mark.parametrize("lane_root", integrity.LANE_ROOTS)
@bite("forge.md:F62")
def test_lane_disjoint_reports_staging_residue(tmp_path: Path, lane_root: str) -> None:
    build_bundle(tmp_path, BUNDLE_UUID, lane_root="staging")
    build_bundle(tmp_path, BUNDLE_UUID, lane_root=lane_root)

    findings = integrity.check_lane_disjoint(str(tmp_path))

    assert [item.code for item in findings] == ["STAGING_RESIDUE"]
    assert findings[0].message == (
        f'staging-residue: bundle uuid "{BUNDLE_UUID}" is resident under staging/ and {lane_root}/'
    )


def test_lane_disjoint_still_reports_lane_overlap(tmp_path: Path) -> None:
    for lane_root in integrity.LANE_ROOTS:
        build_bundle(tmp_path, BUNDLE_UUID, lane_root=lane_root)

    findings = integrity.check_lane_disjoint(str(tmp_path))

    assert [item.code for item in findings] == ["LANE_OVERLAP"]
    assert findings[0].message == (
        f'lane-overlap: bundle uuid "{BUNDLE_UUID}" is resident under lane roots '
        '"samples/" and "delivery/"'
    )


def test_lane_disjoint_reports_all_three_root_collisions(tmp_path: Path) -> None:
    for lane_root in ("staging", *integrity.LANE_ROOTS):
        build_bundle(tmp_path, BUNDLE_UUID, lane_root=lane_root)
...
def test_parent_tree_requires_staging_root(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    build_parent(root)
    ...
    assert any('missing shared directory "staging/"' in item.message for item in findings)


def test_staging_is_a_known_shared_root(tmp_path: Path) -> None:
    findings = integrity.check_unknown_paths(str(tmp_path / "contract.md"), "`staging/`")

    assert findings == []


def test_bundle_layout_accepts_staged_bundle_without_trajectories(tmp_path: Path) -> None:
    build_bundle(tmp_path, BUNDLE_UUID, lane_root="staging")
...
def test_bundle_layout_checks_staging_bytes_even_with_retained_verdict(tmp_path: Path) -> None:
    bundle = build_bundle(tmp_path, BUNDLE_UUID, lane_root="staging")
    (bundle / "task.toml").unlink()
    verdict = tmp_path / ".audit" / "verdicts" / f"{BUNDLE_UUID}.json"
    verdict.parent.mkdir(parents=True)
    verdict.write_text(
        json.dumps({"uuid": BUNDLE_UUID, "bundle_digest": "a" * 64, "outcome": "retained"}),
        encoding="utf-8",
    )
...
@pytest.mark.parametrize(
    ...
    [
        json.dumps({"uuid": BUNDLE_UUID, "bundle_digest": "a" * 64, "outcome": "retained"}),
        json.dumps({"uuid": BUNDLE_UUID, "bundle_digest": "a" * 64, "outcome": "clean"}),
        None,
        '{"outcome":"retained"}',
        json.dumps({"uuid": "other", "bundle_digest": "a" * 64, "outcome": "retained"}),
        json.dumps({"uuid": BUNDLE_UUID, "bundle_digest": "A" * 64, "outcome": "retained"}),
        json.dumps({"uuid": BUNDLE_UUID, "bundle_digest": "a" * 63, "outcome": "retained"}),
        json.dumps({"uuid": BUNDLE_UUID, "bundle_digest": 64, "outcome": "retained"}),
        json.dumps(
            {
                "uuid": BUNDLE_UUID,
                "bundle_digest": "a" * 64,
...
def test_bundle_layout_requires_source_files_regardless_of_verdict(
    tmp_path: Path, verdict_text: str | None
) -> None:
    (tmp_path / "samples" / BUNDLE_UUID / "trajectories").mkdir(parents=True)
    if verdict_text is not None:
        verdict = tmp_path / ".audit" / "verdicts" / f"{BUNDLE_UUID}.json"
        verdict.parent.mkdir(parents=True)
        verdict.write_text(verdict_text, encoding="utf-8")
...
def test_samples_readme_ignores_staged_bundles(tmp_path: Path) -> None:
    build_bundle(tmp_path, BUNDLE_UUID, lane_root="staging")

    findings = integrity.check_samples_readme(str(tmp_path))

    assert findings == []


@pytest.mark.parametrize("has_register", [False, True])
def test_staging_bundles_are_excluded_from_ship_checks(tmp_path: Path, has_register: bool) -> None:
    build_bundle(tmp_path, BUNDLE_UUID, lane_root="staging")
    if has_register:
        (tmp_path / "staging" / "README.md").write_text(
            f"| {BUNDLE_UUID} | SHIP |\n", encoding="utf-8"
        )

    findings = integrity.check_bundle_manifest_signature(
        str(tmp_path), EVALUATION_TIME, StubBackend()
    )
...
def test_bundle_layout_accepts_in_bundle_trajectories(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    build_bundle(root, BUNDLE_UUID)
    assert integrity.check_bundle_layout(str(root)) == []


def test_bundle_layout_rejects_bundle_without_trajectories(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    build_bundle(root, BUNDLE_UUID)
    (root / "samples" / BUNDLE_UUID / "trajectories").rmdir()
    assert any(
...
def test_bundle_layout_rejects_trajectories_without_source_files(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    (root / "samples").mkdir(parents=True)
    (root / "samples" / BUNDLE_UUID / "trajectories").mkdir(parents=True)
    expected = 'missing required bundle file "task.toml"'
    assert any(expected in finding.message for finding in integrity.check_bundle_layout(str(root)))


def test_bundle_layout_accepts_trajectories_in_delivery_lane(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    (root / "samples").mkdir(parents=True)
...
def test_bundle_layout_rejects_delivery_trajectories_without_source_files(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    (root / "delivery").mkdir(parents=True)
    (root / "delivery" / BUNDLE_UUID / "trajectories").mkdir(parents=True)

    assert any(
        'missing required bundle file "task.toml"' in finding.message
        for finding in integrity.check_bundle_layout(str(root))
    )


def test_bundle_layout_does_not_require_harbor_files_in_trajectories(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    build_bundle(root, BUNDLE_UUID)
    trajectories = root / "samples" / BUNDLE_UUID / "trajectories"
    assert not (trajectories / "task.toml").exists()
    assert not (trajectories / "solution" / integrity.TRUTH_FILE).exists()
    assert integrity.check_bundle_layout(str(root)) == []


def test_parent_tree_requires_samples_and_delivery_directories(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    build_parent(root)
    (root / "samples").rmdir()
    (root / "delivery").rmdir()
    ...
    expected = 'missing shared directory "delivery/"'
...
def test_run_parent_registers_all_bundle_checks(tmp_path: Path) -> None:
    root = tmp_path / "parent"
    build_parent(root)
    build_bundle(root, "not-a-uuid", lane_root="samples")
    build_bundle(root, BUNDLE_UUID, lane_root="samples")
    ...
    assert [
        ...
        integrity.check_harness_config,
        ...
        *[check for _name, check in integrity.HARBOR_CHECKS],
        *[check for _name, check in integrity.PIPELINE_CHECKS],
        integrity.check_epochs,
        *[
            check
            for _name, check in integrity.SABOTAGE_CHECKS
            if check.__name__ != "check_disposition_binding"
        ],
        *[check for _name, check in integrity.SENTINEL_CHECKS],
        *[check for name, check in integrity.GATE_CHECKS if name == "check_gates_installed"],
        *integrity.FINAL_AUTHORIZATION_CHECKS,
        *[check for _name, check in integrity.RELEASE_CHECKS],
        *integrity.FINAL_GATE_CHECKS,
    ] == integrity.PARENT_CHECKS
    assert any("not a canonical lowercase uuid5" in finding.message for finding in findings)


def layout_git(root: Path, *args: str) -> str:
    return run_test_git(
        list(args),
        cwd=root,
        extra_env={
...
@pytest.fixture
def layout_parent(tmp_path: Path) -> Path:
    root = tmp_path / "layout-parent"
    root.mkdir()
    layout_git(root, "init", "-q", "-b", "main")
    seed = tmp_path / "seed"
    seed.mkdir()
    layout_git(seed, "init", "-q", "-b", "main")
    (seed / "README.md").write_text("fixture\n", encoding="utf-8")
    layout_git(seed, "add", "README.md")
    layout_git(seed, "-c", "commit.gpgsign=false", "commit", "-qm", "fixture")
    for entry in layout.LAYOUT_ROSTER:
        bare = tmp_path / f"{entry.path}.git"
        layout_git(tmp_path, "clone", "--bare", str(seed), str(bare))
        layout_git(
            root,
            "-c",
        ...
        layout_git(root, "config", "-f", ".gitmodules", f"submodule.{entry.path}.update", "merge")
    (root / ".gitignore").write_text(".secrets/\n.trinity-runtime/\n", encoding="utf-8")
    layout_git(root, "add", ".gitmodules", ".gitignore")
    layout_git(root, "-c", "commit.gpgsign=false", "commit", "-qm", "parent fixture")
    return root


def test_run_parent_registers_check_parent_layout_after_the_submodule_check() -> None:
    checks = integrity.QUALIFICATION_CHECKS

    following = checks[checks.index(integrity.check_parent_submodule) + 1]

    assert following is integrity.check_parent_layout


def test_check_parent_layout_is_silent_outside_a_git_repository(tmp_path: Path) -> None:
    root = tmp_path / "plain"
...
def test_check_parent_layout_accepts_registered_distinct_roots(layout_parent: Path) -> None:
    findings = integrity.check_parent_layout(str(layout_parent))

    assert findings == []


@pytest.mark.parametrize(
    ("commands", "writes", "code", "offending"),
    [
        ([("update-index", "--force-remove", "samples")], [], "PLAIN_DIRECTORY", "samples"),
        (
            [("config", "-f", ".gitmodules", "--remove-section", "submodule.samples")],
            [],
            "SUBMODULE_MISSING",
            "samples",
        ),
        (
            [("config", "-f", ".gitmodules", "submodule.samples.branch", "other")],
            [],
            "BRANCH_INVALID",
            ".gitmodules",
        ),
        (
            [("config", "-f", ".gitmodules", "--unset", "submodule.samples.url")],
            [],
            "GITMODULES_INVALID",
            ".gitmodules",
        ),
        ([], [("samples/HARDNESS.md", "private")], "BOUNDARY_LEAK", "samples"),
        ([], [("deliverables/old.txt", "retired")], "RETIRED_ROOT", "deliverables"),
        ([], [(".gitignore", ".secrets/\n")], "RUNTIME_UNIGNORED", ".gitignore"),
        (
            [
                ("update-index", "--force-remove", "samples"),
                ("update-index", "--add", "--cacheinfo", "100644", "BLOB", "samples/task.txt"),
            ],
            [],
            "PARENT_TRACKS_BUNDLE",
            "samples",
        ),
        (
            [("update-index", "--add", "--cacheinfo", "160000", "HEAD", "extra")],
            [],
            "UNKNOWN_SUBMODULE",
            "extra",
        ),
        ...
        (
            [("update-index", "--add", "--cacheinfo", "100644", "BLOB", ".secrets/key")],
            [],
            "BOUNDARY_LEAK",
            ".secrets/key",
        ),
...
def test_check_parent_layout_reports_broken_boundary(
    layout_parent: Path,
    ...
    substitutions = {
        "HEAD": layout_git(layout_parent, "rev-parse", "HEAD"),
        "BLOB": layout_git(layout_parent, "rev-parse", "HEAD:.gitignore"),
    }
    for command in commands:
        layout_git(layout_parent, *(substitutions.get(arg, arg) for arg in command))
    for relative, text in writes:
        path = layout_parent / relative
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(text, encoding="utf-8")

    findings = integrity.check_parent_layout(str(layout_parent))

    matching = [item for item in findings if item.code == f"PARENT_LAYOUT_{code}"]
    assert len(matching) == 1
    assert matching[0].path == str(layout_parent / offending)
    assert matching[0].line == 0
    assert matching[0].severity == integrity.Severity.ERROR


@pytest.mark.parametrize("relative", ["staging", "samples/link"])
def test_check_parent_layout_refuses_symlinks(layout_parent: Path, relative: str) -> None:
    path = layout_parent / relative
    if path.is_dir():
        shutil.rmtree(path)
    path.symlink_to(layout_parent, target_is_directory=True)

    findings = integrity.check_parent_layout(str(layout_parent))

    assert [item.path for item in findings if item.code == "PARENT_LAYOUT_SYMLINK"] == [str(path)]


def test_check_parent_layout_reports_unreadable_index(layout_parent: Path) -> None:
    (layout_parent / ".git" / "index").write_bytes(b"invalid index")

    findings = integrity.check_parent_layout(str(layout_parent))

    assert [item.code for item in findings] == ["PARENT_LAYOUT_SCAN_TRUNCATED"]


def test_check_parent_layout_reports_scan_limit(
    layout_parent: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    original = layout.scan_forbidden

    def bounded(checkout: Path, forbidden: tuple[str, ...]) -> layout.ScanResult:
        return original(checkout, forbidden, max_entries=0 if checkout.name == "samples" else 100)

    monkeypatch.setattr(layout, "scan_forbidden", bounded)
...
HARNESS_CONFIG_FIXTURES = Path(__file__).parent / "fixtures" / "harness_config"
PLANTED_HARNESS_CONFIGS: dict[str, HarnessConfigFailureReason] = {
    "absent": HarnessConfigFailureReason.HARNESS_CONFIG_MISSING,
    "malformed": HarnessConfigFailureReason.HARNESS_CONFIG_MALFORMED,
    "drifted": HarnessConfigFailureReason.HARNESS_CONFIG_DRIFT,
    "unapproved": HarnessConfigFailureReason.HARNESS_CONFIG_UNAPPROVED,
    "inadequate": HarnessConfigFailureReason.HARNESS_CONFIG_INADEQUATE,
    "schema_unsupported": HarnessConfigFailureReason.HARNESS_CONFIG_SCHEMA_UNSUPPORTED,
    "field_unknown": HarnessConfigFailureReason.HARNESS_CONFIG_FIELD_UNKNOWN,
    "field_absent": HarnessConfigFailureReason.HARNESS_CONFIG_FIELD_ABSENT,
    "value_invalid": HarnessConfigFailureReason.HARNESS_CONFIG_VALUE_INVALID,
    "negative_headroom": HarnessConfigFailureReason.HARNESS_CONFIG_NEGATIVE_HEADROOM,
    "model_id_ambiguous": HarnessConfigFailureReason.HARNESS_CONFIG_MODEL_ID_AMBIGUOUS,
}
HARNESS_CONFIG_REFUSAL_CLASSES = ("absent", "malformed", "drifted", "unapproved", "inadequate")


def harness_config_codes(case: str) -> list[str]:
    return [
        item.code for item in integrity.check_harness_config(str(HARNESS_CONFIG_FIXTURES / case))
    ]


def test_harness_config_check_is_registered() -> None:
    # Given the parent qualification roster.
    checks = integrity.QUALIFICATION_CHECKS
    # When locating the pilot registry that binds a harness configuration digest.
    position = checks.index(integrity.check_pilot_attempt_registry) + 1
    # Then the configuration that digest names is qualified next, exactly once.
    assert checks[position] is integrity.check_harness_config
    assert checks.count(integrity.check_harness_config) == 1


def test_harness_config_clean_fixture_is_clean() -> None:
    # Given an approved, adequate configuration mirrored consistently.
    # When qualifying the parent.
    findings = integrity.check_harness_config(str(HARNESS_CONFIG_FIXTURES / "clean"))
    # Then nothing is refused.
    assert findings == []


@pytest.mark.parametrize("case", HARNESS_CONFIG_REFUSAL_CLASSES)
def test_harness_config_planted_fixtures_fire(case: str) -> None:
    # Given one planted refusal class.
    # When qualifying the parent.
    codes = harness_config_codes(f"planted/{case}")
    # Then exactly that refusal fires, under the module's own vocabulary.
    assert codes == [PLANTED_HARNESS_CONFIGS[case]]


def test_every_harness_config_reason_is_reachable() -> None:
    # Given every planted sub-fixture.
    observed = {
        code for case in PLANTED_HARNESS_CONFIGS for code in harness_config_codes(f"planted/{case}")
    }
    # When comparing the emitted codes with the closed refusal vocabulary.
    # Then no member can be registered inert and no stray code is emitted.
    assert observed == {str(member) for member in HarnessConfigFailureReason}


def test_harness_config_refuses_symlinked_seed_directory(tmp_path: Path) -> None:
    # Given a clean parent whose seed harness is redirected through a symbolic link.
    root = tmp_path / "parent"
    shutil.copytree(HARNESS_CONFIG_FIXTURES / "clean", root)
    elsewhere = tmp_path / "elsewhere"
    shutil.move(str(root / ".seed"), str(elsewhere))
    (root / ".seed").symlink_to(elsewhere, target_is_directory=True)
    # When qualifying the parent.
    findings = integrity.check_harness_config(str(root))
    # Then the redirected harness is refused instead of being read through.
    assert [item.code for item in findings] == [HarnessConfigFailureReason.HARNESS_CONFIG_MISSING]
    assert findings[0].path == str(root / ".seed")


def test_harness_config_exempts_parent_without_config(tmp_path: Path) -> None:
    # Given a parent carrying neither half of the harness configuration.
    root = tmp_path / "parent"
    write_clean_parent(root, lambda _: None)
    # When qualifying the parent.
    findings = integrity.check_harness_config(str(root))
    # Then absence of both halves is an exemption, never a refusal.
    assert findings == []


@pytest.mark.parametrize("case", ["both_empty", "empty_plus_absent"])
def test_harness_config_empty_is_malformed(case: str) -> None:
    # Given: an empty configuration is present, not an absent declaration.
    root = HARNESS_CONFIG_FIXTURES / "planted" / case
    # When: qualifying through the public parent check.
    findings = integrity.check_harness_config(str(root))
    # Then: empty bytes cannot receive the genuinely-absent exemption.
    assert HarnessConfigFailureReason.HARNESS_CONFIG_MALFORMED in [x.code for x in findings]
