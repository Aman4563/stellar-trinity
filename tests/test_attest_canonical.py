...
from tools.attest.canonical import CANONICAL_PROFILE, parse_json
...
def test_oversized_json_integer_is_a_controlled_failure(prefix: bytes) -> None:
    with pytest.raises(CanonicalizationError, match="supported size"):
        parse_json(prefix + (b"9" * 4301))
