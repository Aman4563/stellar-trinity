...
The backend namespace parameter is drawn from the closed set `{"trinity.attestation.v1", "git"}`, defaults to `trinity.attestation.v1`, and requires `namespaces="trinity.attestation.v1,git"` in the signer policy when a principal signs for both purposes.
