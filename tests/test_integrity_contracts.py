...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            integrity.check_fidelity_closure,
            "FORGE.md",
            "Record the compaction event in the ledger.",
            "FORGE never records a compaction event and never reads a detector outcome.",
            "fidelity-detector term",
        ),
    ],
    ids=[
        "retired-name",
        ...
        "fidelity-term-prohibited-in-author",
    ],
)
...
def test_fidelity_closure_ignores_crucible() -> None:
    planted = "Record the compaction event in the ledger."
    assert integrity.check_fidelity_closure("FORGE.md", planted)
    assert integrity.check_fidelity_closure("CRUCIBLE.md", planted) == []
    assert integrity.check_fidelity_closure("ENGRAM.md", planted) == []


def test_fidelity_closure_is_registered() -> None:
    assert integrity.check_fidelity_closure in integrity.CONTRACT_CHECKS


def test_new_dispositions_parse() -> None:
    text = "Cap at `HOLD:SUPPRESSED_MEASUREMENT` or refuse with `BLOCK:INVALID_PILOT`."
    assert integrity.check_disposition_vocab("FORGE.md", text) == []
...
def tool_reimplemented_block() -> str:
    blocks: list[str] = [
        block for block in integrity.SHARED_BLOCKS if "TOOL_REIMPLEMENTED" in block
    ]
    assert blocks, integrity.SHARED_BLOCKS
    return blocks[0]


def test_tool_reimplemented_sentence_is_shared_byte_for_byte() -> None:
    block = tool_reimplemented_block()
    drift = f'no longer states "{block[:70]}"'
    root = Path(__file__).parents[1]
    for name in integrity.CONTRACTS:
        text = (root / name).read_text(encoding="utf-8")
        findings = integrity.check_shared_blocks(name, text)
        assert not any(drift in finding.message for finding in findings), (name, findings)


def test_a_one_word_drift_in_the_shared_sentence_is_a_finding() -> None:
    block = tool_reimplemented_block()
    planted = block.replace("never re-implement it", "rarely re-implement it", 1)
    assert planted != block, block
    assert_text_pair(
        integrity.check_shared_blocks,
        "CRUCIBLE.md",
        planted,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "* @Ethara-Ai/research",
        "samples/<uuid>/trajectories/inspector.html",
        "research/",
    ],
    ids=[
        "shell-just",
...
def test_bare_dynamic_root_is_bound_by_a_templated_path_in_the_document() -> None:
    text = "Use `samples/`. Write `samples/<uuid>/trajectories/`."
...
def test_refusal_code_prefix_is_not_a_disposition_token() -> None:
    clean = integrity.check_disposition_vocab("FORGE.md", "Refuse as `SAB_DISPOSITION_UNBOUND`.")
    assert clean == []
    planted = integrity.check_disposition_vocab("FORGE.md", "Refuse as `SABOTAGE_UNBOUND`.")
    assert any("unknown status token" in item.message for item in planted)


def test_tool_prefix_names_a_coverage_gap_and_not_a_disposition() -> None:
    clean = integrity.check_disposition_vocab("ENGRAM.md", "Record `TOOL_REIMPLEMENTED`.")
    assert clean == []
    planted = integrity.check_disposition_vocab("ENGRAM.md", "Record `TOOLING_REIMPLEMENTED`.")
    assert any("unknown status token" in item.message for item in planted)
...
def test_install_root_is_a_known_hidden_root_and_strangers_are_not() -> None:
    clean = "Genesis reads the receipts under `.trinity-install/protection/` and records gaps.\n"
    assert integrity.check_unknown_paths("ENGRAM.md", clean) == []
    planted = "Genesis reads the receipts under `.stranger-root/protection/` and records gaps.\n"
    assert any(
        'unknown root ".stranger-root/"' in finding.message
        for finding in integrity.check_unknown_paths("ENGRAM.md", planted)
    )
