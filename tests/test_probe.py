...
def run_probe(cwd: Path, *arguments: str) -> subprocess.CompletedProcess[str]:
...
def test_prepare_is_byte_deterministic_for_the_same_seed(tmp_path: Path) -> None:
    ...
    shutil.copytree(BUNDLES, tmp_path / "bundles")

    first_run = run_probe(tmp_path, "prepare", "./bundles", "./first.json", "--seed", "fixed-seed")
    second_run = run_probe(
        tmp_path, "prepare", "./bundles", "./second.json", "--seed", "fixed-seed"
    )
...
def test_score_reports_exact_match_after_normalization_math(tmp_path: Path) -> None:
    ...
    result = run_probe(tmp_path, "score", "./manifest.json", "./responses.json")
...
def test_score_refuses_malformed_inputs_without_a_partial_report(tmp_path: Path) -> None:
    ...
    malformed_manifest = run_probe(tmp_path, "score", "./manifest.json", "./responses.json")
    ...
    malformed_responses = run_probe(tmp_path, "score", "./manifest.json", "./responses.json")
