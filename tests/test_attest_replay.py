...
@pytest.mark.parametrize(
    "project_root,replay_store",
    [
        ("candidate", "./state/replay.jsonl"),
        ("/candidate", "./state/replay.jsonl"),
        ("./../candidate", "./state/replay.jsonl"),
        ("./candidate", "state/replay.jsonl"),
        ("./candidate", "/state/replay.jsonl"),
...
def test_gate_policy_rejects_noncanonical_authored_paths(
    tmp_path: Path, project_root: str, replay_store: str
) -> None:
    ...
    with pytest.raises(ValueError):
        attest_replay.load_gate_policy(policy, authority_root=tmp_path)
