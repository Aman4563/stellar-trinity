...
| File | Scope | Tests |
| --- | --- | --- |
| `forge.md` | FORGE structural constants, phase ceilings, Harbor bundle layout, delivery attribution, task lane routing, stratified inference, band grants, the `harness/` benchmark extension, the thirty-slot batch, and measurement fidelity | 77 |
| `engram.md` | ENGRAM append-only, human-write-only, evidence, `brief/`, feedback chain, task lane shell, inferred-hardness projection, and design-return rules | 47 |
| `crucible.md` | Gate integrity, fail-closed floor, instrument liveness, contamination, judge discipline, rubric, provenance and reward integrity, the task lane partition, and execution fidelity | 88 |
| `shared.md` | Firewall separation, repository structure, rules promoted from consequence-free musts, orchestration obligations, the deferred register, prompt capture and chain integrity, the adversarial operator and delivery integrity, the parent submodule roster, the multi-runner merge, the gate entry point, and disposition vocabulary | 169 |
...
| --- | --- |
| Total tests | 381 |
| Fully covered by `just ci` today | 152 |
| Partially covered | 1 |
| Never verified by anything | 228 |
| Line-anchored contract citations bound in `citations.lock` | 312 |

Five of the 381, the `D` group in `shared.md`, exist only if the merged Axis plus Reason disposition encoding is adopted. Without that decision the suite is 376.
